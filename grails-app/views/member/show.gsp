	...
	<body>
		...
		<div id="show-member" class="content scaffold-show" role="main">
			...
			<ol class="property-list member">
				...
				<g:if test="${memberInstance?.organization}">
				<li class="fieldcontain">
					<span id="organization-label" class="property-label"><g:message code="member.organization.label" default="Organization" /></span>
					
						<span class="property-value" aria-labelledby="organization-label"><g:link controller="organization" action="show" id="${memberInstance?.organization?.id}">${memberInstance?.organization?.encodeAsHTML()}</g:link></span>
